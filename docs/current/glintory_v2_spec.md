...
```json
{
  "facts": [
    {
      "statement": "単一の事実を述べる一文(日本語、最大120文字)",
      "topic": "統制語彙から1つ",
      "entities": ["固有名詞", "..."],
...
> [!IMPORTANT]
> **抽出は「要約」ではない。** 日経新聞要約翻案事件では、記事を要約して翻訳・配信する行為に翻案権侵害が認められている([権利文書](glintory_v2_rights.md) §1.2)。保護されないのは個々の事実だが、**どの事実を選びどう組み立てたかは保護される**。したがって:
>
> - `statement` は「誰が・いつ・何を」の**原子的な単一の事実**に限る。複数の事実を含む叙述的な文、記事の要約、リード文の言い換えを生成してはならない
> - **1つの raw_item から抽出する fact は最大3件**(`GEMINI_MAX_FACTS_PER_ITEM`)。上限を設けるのは、記事の事実の選択と構成をそのまま再現することを構造的に防ぐため
> - プロンプトに「要約」「まとめ」に相当する語を含めない
...
詳細と根拠は[権利文書](glintory_v2_rights.md)。Glintory 側で強制するのは以下。

1. **画像を取得・保存しない。** R2 に保存するのは HTML と API レスポンスのみで、`<img>` の参照先を取得してはならない。キャラクター画像・ゲーム画面・アニメ場面写真はいずれの権利者ガイドラインでも広告収益のある法人メディアへの許諾が確認できていない
2. **`evidence_span` と `source_title` を Loka に渡さない。** 前者は出典検証専用、後者は原見出しへの依拠を構造的に防ぐため(YOL事件)。どちらも Screen・重複排除では内部的に使用してよい
3. **抽出は原子的な事実に限る**(§6.1)
4. **30条の4 の射程を超えない。** Glintory は情報解析工程であり、配信物の表現を生成しない。中立な事実の言い換え(`statement`)までが責務で、記事の文体・構成・見出しは Loka の責務である
