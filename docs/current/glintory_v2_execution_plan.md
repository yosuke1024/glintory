...
> [!IMPORTANT]
> **本計画は2026-08-22に保留となった。** Phase 0(設計)は完了、Phase 1(実装)には未着手。経緯・理由・再開手順は[意思決定記録](glintory_v2_decision_log.md)を参照すること。本書のフェーズ計画とスケジュールはそのまま実行できる状態にはない。
...
> [!WARNING]
> **改訂1の反省(2026-08-22)**: 初版の基準は「法的安全性」「機械可読性」「一次情報」を最適化しており、**読者が読みたいかどうかが基準に入っていなかった**。その結果、官公庁・自治体の告知を中心とする、条件は全て満たすが誰も読まない情報源リストが生成された。さらに「一次情報」が実質「官公庁」にすり替わっていた(任天堂の新作発表も専門メディアの独自取材も一次情報である)。本節はその反省に基づく全面改訂版である。

### 第一基準: 読者価値(これを満たさないものは他の条件を満たしても採用しない)

**タイ・インドネシアに住む一般の人が、自国にいながら「読みたい」と思う日本の情報であること。**

判定は「この記事の見出しが自国語のSNSに流れてきたら、自分はタップするか」で行う。以下は読者価値が高い順の想定テーマ:

1. アニメ・マンガ・ゲーム(東南アジアの最大需要領域)
2. 食・新商品(コンビニスイーツ、ラーメン、期間限定品、菓子)
3. 音楽・芸能・ドラマ・映画
4. 旅行・観光地・季節の風物詩(桜、紅葉、花火、雪)
5. コスメ・スキンケア・雑貨(東南アジアで日本製品の需要が高い)
6. 変わった日本・生活文化(「日本ではなぜXなのか」)

**明示的に読者価値が低いと判断するもの**: 政策・制度の告知、審議会・委員会の開催案内、統計の公表、日本人向けの行政手続き情報、日本人の海外渡航向け情報。

### 第二基準: 情報の鮮度と量

- **1媒体あたり日10本以上の記事が流れること**を目安とする。週次更新の媒体は、季節情報など特別な理由がない限り採用しない。
- 記事化の候補が枯渇しない量が、自律メディアの前提である。

### 第三基準: 情報の質(「一次情報」の正しい定義)

以下のいずれかであること:

- **発信主体による公式発表**(企業の新商品リリース、ゲーム会社の新作発表、施設の公式告知)
- **編集部による独自取材・独自報道**(専門メディアのニュース記事、インタビュー、レビュー)

除外するのは「他媒体の記事をまとめただけのサイト」「引用元を明示しない二次配信」であり、**商業メディアであることは除外理由にならない**。

### 第四基準: 技術要件

- RSS / Atom / 構造化フィードで安定取得できる(初期はスクレイピング依存を避ける)
- robots.txt を尊重できる

### 権利の扱い(基準ではなく設計で担保する)

読者価値の高い情報の多くは商業メディア・企業発であり、利用規約に「無断転載禁止」が定められている。これは**採用可否の基準ではなく、実装で守るべき制約**として扱う。

- 日本の著作権法上、**事実そのものに著作権は生じない**(第10条第2項: 事実の伝達にすぎない雑報及び時事の報道は著作物に該当しない)。保護されるのは表現である。
- したがって Glintory が行う「事実の抽出」と、Loka が行う「独自の表現による記事化 + 出典リンク」は転載にあたらない。これは各国の報道機関が日常的に行っていることと同じ構造である。
- 実装で強制する制約:
  - `evidence_span`(逐語引用)は**内部の検証専用**であり、記事に出力してはならない(供給契約で明示)
  - 見出し・リード文の逐語翻訳を禁止する(表現の模倣にあたりうる)
  - **画像は一切取得・再利用しない**(写真は明確な著作物。Loka の記事画像は自前調達とする)
  - 出典名とリンクを必ず記事に明示する
  - robots.txt 尊重、同一ホストへのリクエスト間隔を空ける

規約リスクが特に高いと判断される媒体は、採用せず同種の代替媒体を探す。事業規模が拡大した段階で、主要媒体については法務レビューを行う。

具体的な情報源リストは [初期情報源リスト](glintory_v2_sources.md) で確定する。
