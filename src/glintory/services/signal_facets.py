...
def extract_signal_facets(
    ...
    product_or_project_terms = [
        kw for kw in product_or_project_keywords if has_word(kw)
    ]
    ...
    structural_completeness = {
        ...
        "gap": "confirmed"
        if (alternative_markers or cost_markers or migration_markers)
        else "missing",
        "mvp": "confirmed"
        if (
            solution_request_markers
            or any(kw in text_lower for kw in ["mvp", "feature", "scope"])
        )
        else "missing",
    }
