...
class OpportunityRebuildService:
    ...
    def _calculate_metrics_and_gate(self, cluster_signals: list[dict[str, Any]]) -> tuple[dict[str, int], bool, str]:
        signals = [item["signal"] for item in cluster_signals]
        if not signals:
            return (
                {"independent_evidence_count": 0, "demand_evidence_count": 0, "source_type_count": 0, "source_domain_count": 0},
        ...
        if independent_count == 1:
            ...
            is_show_hn = any(
                (sig.source and sig.source.source_type == "hackernews" and (sig.title or "").lower().startswith("show hn:"))
        ...
        user_kws = ["customer", "target user", "developer", "target audience", "for developers", "for users", "ユーザー", "顧客", "開発者", "ターゲットユーザー"]
        ...
        mvp_kws = ["mvp", "solution", "feature", "idea", "should", "wish", "提案", "欲しい", "必要", "機能"]
        ...
        is_solo_realistic = not any(has_word(kw, combined_text) for kw in ["enterprise-grade", "multi-tenant", "collaboration", "rbac", "salesforce integration", "large scale", "組織向け", "共同編集", "権限管理"])
        is_heavy_backend_free = not any(has_word(kw, combined_text) for kw in ["heavy backend", "complex backend", "microservices", "kubernetes", "k8s", "large scale database", "heavy server", "重いバックエンド", "マイクロサービス"])
        is_ai_cost_free = not any(has_word(kw, combined_text) for kw in ["heavy api cost", "expensive api", "expensive ai", "high hosting cost", "high running cost", "高額なapi", "ai費用", "高額なホスティング"])
        is_enterprise_sales_free = not any(has_word(kw, combined_text) for kw in ["enterprise sales", "sales cycle", "sales team", "b2b sales", "エンタープライズ営業", "営業チーム", "営業プロセス"])
        ...
        if not quality_passed:
            ...
            return (metrics, False, f"Rejected: Quality gate failed. Missing structural elements or violating constraints. ({details})")
        ...
        if single_sig.signal_role == SignalRole.DEMAND:
            return (metrics, True, "Passed Condition B: Strong detailed single demand with all 5 structure elements.")

        return (metrics, False, "Rejected Condition B: Single evidence is not demand.")

    def rebuild_v2(self, from_version: str = "v1", to_version: str = "v2") -> dict[str, Any]:
        ...
        existing_opps = self.session.query(Opportunity).filter(
            Opportunity.current_scoring_version == to_version
        ).all()

        opp_data_map = {}
        for opp in existing_opps:
            opp_sigs = self.session.query(OpportunitySignal).filter(
            ...
            oldest_sig_id = None
            if opp_sigs:
                sigs = self.session.query(Signal).filter(Signal.id.in_(list(sig_ids))).all()
                if sigs:
            ...
            rep_sig_id = None
            centroid_links = [
                osig for osig in opp_sigs
                if osig.relation_type in (EvidenceRelationType.SUPPORTING, EvidenceRelationType.RELATED)
            ]
            if centroid_links:
                c_sigs = self.session.query(Signal).filter(Signal.id.in_([link.signal_id for link in centroid_links])).all()
                if c_sigs:
                    sorted_c_sigs = sorted(c_sigs, key=lambda s: (s.collected_at, s.id))
                    rep_sig_id = sorted_c_sigs[0].id

            opp_data_map[opp.id] = {
                "opportunity": opp, "sig_ids": sig_ids, "oldest_sig_id": oldest_sig_id, "rep_sig_id": rep_sig_id,
                "public_id": opp.public_id, "first_published_at": opp.first_published_at, "last_published_at": opp.last_published_at,
                "status": opp.status, "created_at": opp.created_at, "public_revision": opp.public_revision,
                "public_content_hash": opp.public_content_hash
            }

        # Reclassify from_version signals
        from_opps = self.session.query(Opportunity).filter(
            Opportunity.current_scoring_version == from_version
        ).all()
        from_opp_ids = [opp.id for opp in from_opps]
        from_opp_sigs = self.session.query(OpportunitySignal).filter(
            OpportunitySignal.opportunity_id.in_(from_opp_ids)
        ).all() if from_opp_ids else []
        from_sig_ids = list({osig.signal_id for osig in from_opp_sigs})

        signals_to_reclassify = self.session.query(Signal).filter(Signal.id.in_(from_sig_ids)).all() if from_sig_ids else []
        for sig in signals_to_reclassify:
            src = sig.source
            src_type = src.source_type if src else "unknown"
            sig.signal_role = classify_signal_role(src_type, sig.signal_type, sig.title, sig.excerpt)
        self.session.flush()
        ...
        for idx, cluster in enumerate(clusters):
            ...
            sorted_cluster_sigs = sorted(cluster_sigs, key=lambda s: (s.collected_at, s.id))
            cluster_oldest_sig_id = sorted_cluster_sigs[0].id if sorted_cluster_sigs else None

            matched_candidates = []
            for _, data in opp_data_map.items():
                is_match = False
                if data["rep_sig_id"] and data["rep_sig_id"] in cluster_sig_ids or cluster_sig_ids & data["sig_ids"] or data["oldest_sig_id"] == cluster_oldest_sig_id:
                    is_match = True
            ...
            if matched_candidates:
                sorted_candidates = sorted(matched_candidates, key=lambda x: x["created_at"])
        ...
        for idx, cluster in enumerate(clusters):
                ...
                if opp:
                    ...
                    for merged in merged_opps.get(canonical_id, []):
                        ...
                        if m_opp.id not in saved_opp_ids:
                            self.session.delete(m_opp)
            else:
                new_id = str(uuid.uuid4())
                public_id = f"opp_{uuid.uuid4().hex}"
                opp = Opportunity(
                    id=new_id, public_id=public_id, public_revision=1, title=title,
                    generation_method="deterministic_cluster", cluster_version=config.cluster_version,
                    last_clustered_at=now, status=OpportunityStatus.INBOX if passed else OpportunityStatus.RESEARCH,
                    evidence_updated_at=now, independent_evidence_count=metrics["independent_evidence_count"],
                    demand_evidence_count=metrics["demand_evidence_count"], source_type_count=metrics["source_type_count"],
                    source_domain_count=metrics["source_domain_count"], gate_version="v2",
                    gate_status="passed" if passed else "rejected", gate_reason=reason, gate_checked_at=now,
            ...
            for sig_info in cluster["signals"]:
                ...
                opp_sig = OpportunitySignal(
                    opportunity_id=canonical_id, signal_id=sig.id,
                    relation_type=sig_info["relation_type"], relevance_score=sig_info["relevance_score"],
                    association_source="clustering", is_excluded=False, updated_at=now,
                )
                self.session.add(opp_sig)

        # Delete unused opportunities
        for opp_id, data in opp_data_map.items():
            if opp_id not in saved_opp_ids:
                opp_to_del = data["opportunity"]
                if opp_to_del in self.session:
                    self.session.delete(opp_to_del)
        ...
        v2_opps = self.session.query(Opportunity).filter(
            Opportunity.current_scoring_version == to_version
        ).all()

        for opp in v2_opps:
            links = self.session.query(OpportunitySignal, Signal).join(
                Signal, OpportunitySignal.signal_id == Signal.id
            ).filter(
                OpportunitySignal.opportunity_id == opp.id,
