    ...
    if isinstance(data, dict):
        for k, v in data.items():
            ...
            for fk in forbidden_keys:
                if fk in k_lower:
                    errors.append(
                        f"Security violation: key '{current_path}' contains forbidden term '{fk}'"
                    )
        ...
        for pattern, desc in security_patterns:
            if pattern.search(data):
                errors.append(
                    f"Security violation: value at '{path}' matches credential pattern '{desc}'"
                )
    return errors
...
def validate_public_contract(data_dir: str) -> list[str]:
    ...
    # Validate opportunities.json duplicate IDs
    list_ids = [item.public_id for item in opp_list.items]
    if len(list_ids) != len(set(list_ids)):
        errors.append("Duplicate public_id detected in opportunities.json list")

    # Validate feeds/jurypress.json duplicate IDs
    feed_ids = [item.public_id for item in jurypress.items]
    if len(feed_ids) != len(set(feed_ids)):
        errors.append("Duplicate public_id detected in feeds/jurypress.json")
    ...
    for df in detail_files:
        ...
        if detail.public_id != public_id:
            errors.append(
                f"Public ID mismatch in detailed JSON: filename has {public_id}, but JSON has {detail.public_id}"
            )
            continue
        ...
        if detail.content_hash != recalculated_hash:
            errors.append(
                f"Content hash integrity failure for {public_id}. Detail claims {detail.content_hash}, but re-calculated is {recalculated_hash}"
            )
        ...
        if detail.evidence:
            for ev in detail.evidence:
                if ev.excerpt and len(ev.excerpt) > 500:
                    errors.append(
                        f"Detail {public_id} evidence {ev.signal_id} has excerpt exceeding 500 characters"
                    )
                if not (ev.url.startswith("http://") or ev.url.startswith("https://")):
                    errors.append(
                        f"Detail {public_id} evidence {ev.signal_id} has invalid URL format: '{ev.url}'"
                    )
        ...
        if detail.public_lifecycle == "active":
            ...
            if detail.score is None or detail.gate is None or detail.jurypress is None:
                errors.append(
                    f"Active Opportunity {public_id} is missing score, gate or readiness details"
                )
                continue
            ...
            if (
                ...
                or detail.localization is None
            ):
                ...
                ja_fields = [
                    ...
                    ja.problem,
                    ja.target_user,
                    ja.current_workaround,
                    ja.existing_solution_gap,
                    ja.mvp_direction,
                    ja.why_selected,
                    ja.risks,
                ]
            ...
            if (
                detail.translation_status != "completed"
                or (
                    detail.localization and detail.localization.en.status != "completed"
                )
                or detail.localization is None
            ):
                ...
                en_fields = [
                    ...
                    en.problem,
                    en.target_user,
                    en.current_workaround,
                    en.existing_solution_gap,
                    en.mvp_direction,
                    en.why_selected,
                    en.risks,
                ]
            ...
            if detail.evidence:
                for ev in detail.evidence:
                    if (ev.summary_ja and len(ev.summary_ja.strip()) > 0) or (
                        ev.summary_en and len(ev.summary_en.strip()) > 0
                    ):
                        has_ev_summary = True
                        break
            ...
            if detail.jurypress.ready != ready_recalc:
                errors.append(
                    f"JuryPress readiness mismatch on detail {public_id}: recalc={ready_recalc}, detail={detail.jurypress.ready}"
                )
            if sorted(detail.jurypress.reasons) != sorted(unique_re):
                errors.append(
                    f"JuryPress reasons mismatch on detail {public_id}: recalc={unique_re}, detail={detail.jurypress.reasons}"
                )
            ...
            if detail.retired_at is None or detail.retired_reason is None:
                errors.append(
                    f"Retired detail {public_id} is missing retired_at or retired_reason"
                )
            scanned_tombstones.append(detail)

        elif detail.public_lifecycle == "merged":
            if (
                detail.canonical_public_id is None
                or detail.canonical_detail_url is None
            ):
                errors.append(
                    f"Merged detail {public_id} is missing canonical target links"
                )
            scanned_aliases.append(detail)

        else:
            errors.append(
                f"Invalid public_lifecycle value on detail {public_id}: {detail.public_lifecycle}"
            )

    # Validate Alias duplicate and overlap checks
    active_ids = {d.public_id for d in scanned_active_summaries}
    retired_ids = {d.public_id for d in scanned_tombstones}
    alias_ids = [d.public_id for d in scanned_aliases]

    if len(alias_ids) != len(set(alias_ids)):
        errors.append("Duplicate old_public_id detected in merged alias records")

    conflicts = set(alias_ids) & (active_ids | retired_ids)
    if conflicts:
        errors.append(
            f"Conflict detected: merged alias IDs overlap with active/retired IDs: {list(conflicts)}"
        )
    ...
    for item in opp_list.items:
        ...
        detail_match = next(
            (d for d in scanned_active_summaries if d.public_id == item.public_id), None
        )
        if detail_match:
            if item.content_hash != detail_match.content_hash:
                errors.append(
                    f"Content hash mismatch for {item.public_id} between opportunities.json list ({item.content_hash}) and detail ({detail_match.content_hash})"
                )

            # Check readiness consistency
            if item.jurypress.ready != detail_match.jurypress.ready:
                errors.append(
                    f"JuryPress readiness flag mismatch for {item.public_id} between list ({item.jurypress.ready}) and detail ({detail_match.jurypress.ready})"
                )

            # Check detail_url consistency
            expected_detail_url = (
                f"{base_path}/data/v1/opportunities/{item.public_id}.json"
            )
            if item.detail_url != expected_detail_url:
                errors.append(
                    f"Invalid detail_url for opportunity {item.public_id}: expected '{expected_detail_url}', got '{item.detail_url}'"
                )
    ...
    for feed_item in jurypress.items:
        detail_match = next(
            (
                d
                for d in scanned_jurypress_feed_items
                if d.public_id == feed_item.public_id
            ),
            None,
        )
        if detail_match and feed_item.content_hash != detail_match.content_hash:
            errors.append(
                f"Content hash mismatch for {feed_item.public_id} between JuryPress Feed ({feed_item.content_hash}) and detail ({detail_match.content_hash})"
            )
    ...
    registered_ids = (
        active_ids_from_list
        | {d.public_id for d in scanned_tombstones}
        | {d.public_id for d in scanned_aliases}
    )
    untracked_ids = observed_ids - registered_ids
    if untracked_ids:
        errors.append(
            f"Stray detailed Opportunity JSONs detected: {list(untracked_ids)}"
        )
