...
async def run_score_command(args: argparse.Namespace, runtime: Any) -> int:
    ...
    scoring_version = getattr(args, "score_version", None) or runtime.settings.scoring_version
...
async def run_schedule_command(args: argparse.Namespace, runtime: Any) -> int:
    ...
    try:
        ...
        source = source_repo.get_by_identifier(args.source)
        if not source:
            sys.stderr.write(f"Source not found: {args.source}\n")
            return 2
        source_id = source.id
        ...
        if args.subcommand == "enable":
            try:
                ...
                if args.json:
                    print(json.dumps({"source_id": source_id, "enabled": True}))
                else:
                    print(f"Schedule for source '{sched.source_name}' enabled.")
            except ScheduleNotFoundError:
                sys.stderr.write(f"Schedule for Source {args.source} not found.\n")
                return 2
            return 0
        ...
        if args.subcommand == "disable":
            try:
                ...
                if args.json:
                    print(json.dumps({"source_id": source_id, "enabled": False}))
                else:
                    print(f"Schedule for source '{sched.source_name}' disabled.")
            except ScheduleNotFoundError:
                sys.stderr.write(f"Schedule for Source {args.source} not found.\n")
                return 2
            return 0
    ...
    try:
        ...
        if args.json:
            ...
            for item in diagnostics_list:
                ...
                print(f"  Fetched: {item['fetched_count']}, Persisted: {item['persisted_count']}, Skipped: {item['skipped_count']}, Errors: {item['error_count']}")
                if item["error_type"]:
