...
def sanitize_brief_data(
    ...
    def clean_val(val: Any, default_val: str = "N/A") -> str:
        if not isinstance(val, str) or not val.strip():
            return default_val
        # Replace < and > to [ and ] to avoid HTML tag errors
        val = val.replace("<", "[").replace(">", "]")
        # Replace URLs to avoid URL validation errors
        val = re.sub(r"https?://[^\s/$.?#].[^\s]*", "[URL]", val)
    ...
    # Sanitize english and japanese brief structures
    for lang in ("english", "japanese"):
        ...
        for f in fields:
            ...
            lang_dict[f] = clean_val(lang_dict.get(f), default_val=default)

    # 2. Sanitize evidence_refs
    ...
    clean_refs = []
    for ref in refs:
        if isinstance(ref, str) and ref in input_evidence_ids:
            clean_refs.append(ref)

    if not clean_refs:
        clean_refs = list(input_evidence_ids)
    data["evidence_refs"] = clean_refs

    # 3. Sanitize evidence_summaries
    ...
    if not isinstance(summaries, list) or not summaries:
        ...
        for e in request.evidence:
            summaries.append(
                {
                    "id": e["id"],
                    "summary_en": clean_val(
                        e.get("title") or e.get("content") or "Evidence detail",
                        "Evidence detail",
                    ),
                    "summary_ja": clean_val(
                        e.get("title") or e.get("content") or "証拠の詳細", "証拠の詳細"
                    ),
                }
            )
    else:
        clean_summaries = []
        summary_by_id = {
            s["id"]: s for s in summaries if isinstance(s, dict) and "id" in s
        }

        for e in request.evidence:
            e_id = e["id"]
            if e_id in summary_by_id:
                s = summary_by_id[e_id]
                clean_summaries.append(
                    {
                        "id": e_id,
                        "summary_en": clean_val(
                            s.get("summary_en"), e.get("title") or "Evidence detail"
                        ),
                        "summary_ja": clean_val(
                            s.get("summary_ja"), e.get("title") or "証拠の詳細"
                        ),
                    }
                )
            else:
                clean_summaries.append(
                    {
                        ...
                        "summary_ja": clean_val(
                            e.get("title") or e.get("content") or "証拠の詳細",
                            "証拠の詳細",
                        ),
                    }
                )
        summaries = clean_summaries
    data["evidence_summaries"] = summaries
    ...
    if conf not in ("low", "medium", "high"):
        data["confidence"] = (
            request.confidence
            if request.confidence in ("low", "medium", "high")
            else "low"
        )
