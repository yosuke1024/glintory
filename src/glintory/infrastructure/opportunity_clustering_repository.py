...
class OpportunityClusteringRepository:
    ...
    def load_unassociated_signals(self) -> list[Signal]:
        """Load all signals that are not linked to any opportunity."""
        subq = self.session.query(OpportunitySignal.signal_id)
    ...
    def load_active_opportunities_with_signals(self) -> list[dict]:
        """Load all active opportunities along with their associated signals.

        Returns a list of dicts:
        {
            "opportunity": Opportunity,
            "signals": list[tuple[Signal, float]]  # (Signal, relevance_score)
        }
        """
        opps = (
        ...
        for opp_sig, sig in links:
            opp_to_signals[opp_sig.opportunity_id].append(
                (sig, opp_sig.relevance_score)
            )

        return [
            {"opportunity": opp, "signals": opp_to_signals[opp.id]}
            for opp in opps
        ]
