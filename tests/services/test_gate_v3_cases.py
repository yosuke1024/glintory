...
def test_case_a_multiple_independent_with_demand():
    ...
    sig1 = DummySignal(
        "Looking for a self-hosted alternative to Slack",
        "Slack is too expensive and my team needs privacy",
        "hackernews",
    ...
    sig2 = DummySignal(
        "Show HN: Rufslack - self-hosted Slack alternative",
        "Rufslack is a fast alternative written in Go",
        "hackernews",
...
def test_case_b_multiple_independent_no_demand():
    ...
    sig2 = DummySignal(
        ...
        "Go code",
        "github",
        "https://github.com/user/rufslack",
        SignalRole.SUPPLY,
        SignalType.LAUNCH,
    )
...
def test_case_e_heavy_backend_exclusion():
    ...
    sig = DummySignal(
        "Looking for a team database cluster",
        "We require kubernetes and microservices for large scale operations",
        "hackernews",
    ...
    assert passed_published is False
    assert (
        "Requires heavy backend" in reason
        or "Not suitable for solo developer" in reason
    )
...
def test_case_f_contextual_negative_pass():
    ...
    sig = DummySignal(
        "I hate kubernetes, it is too complex",
        "I want a single binary alternative to k8s for simple deployments",
        "hackernews",
...
def test_case_g_single_show_hn():
    ...
    sig = DummySignal(
        "Show HN: New database alternative",
        "This is my tool",
        "hackernews",
...
def test_case_j_tracking_parameters_normalization():
    ...
    o1 = calculate_evidence_origin(
        "rss", "https://example.com/blog/post?utm_source=twitter&utm_medium=social"
    )
    o2 = calculate_evidence_origin(
        "rss", "https://example.com/blog/post?utm_source=feed&ref=newsletter"
    )
