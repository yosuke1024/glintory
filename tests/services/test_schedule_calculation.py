...
from glintory.services.schedule_calculation import calculate_due_occurrence, DueOccurrence
...
def test_calculate_due_occurrence_validation():
    ...
    # Naive current
    with pytest.raises(ValueError, match="current_next_run_at must be timezone-aware UTC"):
        calculate_due_occurrence(current_next_run_at=current_naive, now=now_utc, interval_minutes=60)

    # Naive now
    with pytest.raises(ValueError, match="now must be timezone-aware UTC"):
        calculate_due_occurrence(current_next_run_at=current_utc, now=now_naive, interval_minutes=60)

    # Invalid interval too small
    with pytest.raises(ValueError, match="interval_minutes must be between 5 and 525600"):
        calculate_due_occurrence(current_next_run_at=current_utc, now=now_utc, interval_minutes=4)

    # Invalid interval too large
    with pytest.raises(ValueError, match="interval_minutes must be between 5 and 525600"):
        calculate_due_occurrence(current_next_run_at=current_utc, now=now_utc, interval_minutes=525601)
