...
def upgrade() -> None:
    ...
    with op.batch_alter_table("opportunities", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("current_workaround_en", sa.Text(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("current_workaround_ja", sa.Text(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("existing_solution_gap_en", sa.Text(), nullable=True)
        )
        batch_op.add_column(
            sa.Column("existing_solution_gap_ja", sa.Text(), nullable=True)
        )
        ...
        batch_op.add_column(
            sa.Column(
                "source_type_count", sa.Integer(), nullable=False, server_default="0"
            )
        )
        batch_op.add_column(
            sa.Column(
                "source_domain_count", sa.Integer(), nullable=False, server_default="0"
            )
        )

    # Data migration: archive/reject existing v1 opportunities
    op.execute(
        "UPDATE opportunities SET status = 'rejected', current_scoring_version = 'v1' WHERE current_scoring_version IS NULL OR current_scoring_version = 'v1';"
    )
